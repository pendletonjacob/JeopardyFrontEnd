...
public class jeopardyGame extends Application {
	...
	private File WrongSound = new File("bin/wrong.mp3");
	...
	public void start(Stage primaryStage) {
		...
		List<Category> categories = DataRepository.getCategoryQuestions();
		...
		{
			int length = categories.get(i).CategoryName.length();
			if(length > 10)
				length = 10;
			String name = categories.get(i).CategoryName.substring(0, length);
			Label cat = new Label(name);
				...
				A1.setOnAction(new EventHandler<ActionEvent>() {
					...
					public void handle(ActionEvent e) {
						...
						if (fileName.equals(answer)) {
							File f = new File("bin/correct.mp3");
							URI u = f.toURI();
		...
		bt2Cat5.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				textArea.setText(
						"The while loop and the do loop are equivalent in their expressive\n power; in other words, you can rewrite a while loop using a do loop,\n and vice versa.");
				A1.setDisable(false);
				A1.setOnAction(new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent e) {
